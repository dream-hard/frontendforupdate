...
const DEFAULT_CURRENCIES = ["USD", "SYP"];

export default function Cart110() {
...
const getCurrencyMeta = useCallback(
  ...
  if (m.symbol || m.name) return `${m.code} ${m.symbol ? `— ${m.symbol}` : ""} ${m.name ? `(${m.name})` : ""}`.trim();
  return m.code;
};
// 
  ...
  const normalizeCurrencyResponse = (data) => {
    ...
    // 1) array of strings -> keep
    if (Array.isArray(data) && data.every((x) => typeof x === "string")) return data;
    // 2) array of objects -> try common keys
    if (Array.isArray(data) && data.every((x) => typeof x === "object")) {
      // prefer code or currency or id
      const mapped = data.map((o) => o.code || o.currency || o.id || Object.values(o)[0]).filter(Boolean);
  ...
  async function fetchcurrencies() {
    try {
      ...
      const codes = normalizeCurrencyResponse(res?.data);
  ...
  const currencyKeys = useMemo(() => {
    return normalizeCurrencyResponse(currencies);
  }, [currencies]);
              ...
              return (
                <div key={itemKey} className="d-flex align-items-center mb-3 p-3 shadow-sm rounded">
                  ...
                  <div className="flex-grow-1">
                    ...
                    <p className="mb-1">
                      السعر للواحدة: {Number(item?.cost_per_one || 0).toFixed(2)} {originalCurrency}
                      <br />
                      وحدة (محولة): {Number(convUnit).toFixed(2)} {selectedCurrency}
                    </p>
                    ...
                    <p className="mb-1 ">
                       المجموع لهذا المنتج
                      <button onClick={()=>{console.log(selectedCurrency)}}>dasf</button>
                      <strong>
                        {Number(converted || 0).toFixed(2)} {selectedCurrency}
                      </strong>
                    </p>
              ...
              <div className="card-body p-2">
                      ...
                      <div key={it.key} className="list-group-item d-flex align-items-center gap-2">
                        ...
                        <div className="flex-grow-1">
                          <div className="d-flex justify-content-between">
                            ...
                            <div style={{ fontSize: 13 }}>{Number(it.totalConv || 0).toFixed(2)} {modalCurrency}</div>
                          </div>
                          <div className="text-muted small d-flex justify-content-between">
                            <div>كمية: {it.qty}</div>
                            <div>وحدة: {Number(it.unitConv || 0).toFixed(2)} {modalCurrency}</div>
                          </div>
                        </div>
                      </div>
                    ))
                ...
                <div className="mb-2">
                  <div className="d-flex justify-content-between">
                    ...
                    <strong>{Number(modalSubtotal || 0).toFixed(2)} {modalCurrency}</strong>
                  </div>
            
                  <hr />
                  <div className="d-flex justify-content-between fs-5">
                    <span>المجموع النهائي</span>
                    <strong>{Number(modalTotal || 0).toFixed(2)} {modalCurrency}</strong>
                  </div>
                </div>

                <div className="small text-muted">عملة العرض: {modalCurrency}</div>
              </div>
            </div>
          </div>
        </div>
